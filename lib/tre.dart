...
class _CartPageState extends State<CartPage> {
  ...
  Future fetchAllCartProductDataDetails() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (mounted) {
        setState(() {
          // ProductService.getInstance.fetchProductDetails(dataAllCartProductList.length);
          fetchProductDetails();
          isLoading = false;
        });
      }
  ...
  void initState() {
    ...
    getInstance();
    print(
        "Lengths::${CartProductService.getInstance().dataAllCartProductList.length}");
    print(
        "Lengths::${ProductService.getInstance().productDetailsDataList.length}");
  ...
  Future<dynamic> getInstance() async {
    return await ProductService.getInstance().fetchProductDetails();
  ...
  @override
  void dispose() {
    // fetchAllCartProductDataDetails().ignore();
    // fetchProductDetails().ignore();
    productDetailsDataList.clear();
    dataAllCartProductList.clear();
    super.dispose();
  }
